...
public class AddEventActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        locationOuput.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView parentView, View childView,
                ...
                MainActivity.eventController.setCoordinate(longitude, latitude);
                MainActivity.eventController.setLocationName(addressName);
       ...
       saveButton.setOnClickListener(new View.OnClickListener() {
            ...
            if (theComment.length() > 20) {
                ...
                try {
                    MainActivity.eventController.setComment(theComment);
                    SimpleDateFormat format = new SimpleDateFormat("dd-MM-yyyy");
                    format.setLenient(FALSE);
                    eventDate = format.parse(date.getText().toString());
                    MainActivity.eventController.setDate(eventDate);
                    if (locationInput.getText().toString().length() == 0){
                        MainActivity.eventController.setLocationName("");
                        MainActivity.eventController.setCoordinate(0, 0);
                    }
                    if (!checkDuplicate(eventDate)) {
                        ...
                        //addEventDone();
                        MainActivity.eventController.saveAddEvent();
                        MainActivity.achievementController.updateNewYearsResolution();
                        MainActivity.achievementController.updateBusyBeaver();
                        MainActivity.achievementController.updateWeekendWarrior();
                        MainActivity.achievementController.updateFirstEvent();
                        MainActivity.habitList.sort();
    ...
    private void setDateTimeField() {
     ...
     private boolean checkDuplicate(Date edate) {
        ...
        df.format(result);
        HabitEvent event  = MainActivity.eventController.returnEvent();

        for (int i = 0; i < MainActivity.eventList.getSize(); i++){
            Date getDate = MainActivity.eventController.getDate(i);
            String title = MainActivity.eventController.getType(i).getTitle();
            if (title.equals(habit.getTitle())  && getDate.equals(edate)){
                MainActivity.eventController.setEvent(event);
                return FALSE;
            }
        ...
        switch (requestType) {
            ...
            case IMAGE_REQUEST_CODE: {
                ...
                if (ContextCompat.checkSelfPermission(this, permission) != PackageManager.PERMISSION_GRANTED) {
                    ...
                    MainActivity.eventController.setImage(selectImage);
                }
                return;
    ...
    protected void onStart(){
        ...
        habit = MainActivity.habitList.getHabit(position);
        MainActivity.eventController.addEvent(habit);
        MainActivity.eventController.setUserId(habit.getUserID());
    }
